<div style="overflow-x: hidden !important">
    @include('livewire.sanitasi-jamban.create')
    @include('livewire.sanitasi-jamban.update')
    <div class="row">
        <div class="col-12">
                            ...
                            <tbody>
                                @foreach ($data as $dt)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <p class="text-xs font-weight-bold mb-0">{{ $dt->jml_kk }}</p>
                                        </td>
                                        <td class="text-center">
                                            <p class="text-xs font-weight-bold mb-0">{{ $dt->jml_akses_jamban }}</p>
                                        </td>
                                        <td class="text-center">
                                            <p class="text-xs font-weight-bold mb-0">{{ round(($dt->jml_akses_jamban / $dt->jml_kk)*100 , 1) }}</p>
                                        </td>
                                        <td class="text-center">
                                            <a href="#" wire:click="edit({{ $dt->id_sanitasi }})" class="mx-3" data-bs-toggle="tooltip"
                                                data-bs-original-title="Edit" data-toggle="modal" data-target="#updateModal">
                                                <i class="fas fa-edit text-secondary"></i>
                                            </a>
                                            <span>
                                            <a href="#" wire:click="deleteConfirm({{ $dt->id_sanitasi }})"  data-bs-toggle="tooltip"
