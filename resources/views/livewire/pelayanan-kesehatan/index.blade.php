                            ...
                            <tbody>
                                @foreach ($data as $dt)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <p class="text-xs font-weight-bold mb-0">{{ $dt->jml_balita }}</p>
                                        </td>
                                        <td class="text-center">
                                            <p class="text-xs font-weight-bold mb-0">{{ $dt->jml_balita_sehat }}</p>
                                        </td>
                                        <td class="text-center">
                                            <p class="text-xs font-weight-bold mb-0">{{ round(($dt->jml_balita_sehat / $dt->jml_balita)*100 , 1) }}</p>
                                        </td>
                                        <td class="text-center">
                                            <a href="#" wire:click="edit({{ $dt->id_pelayanan }})" class="mx-3" data-bs-toggle="tooltip"
                                                data-bs-original-title="Edit" data-toggle="modal" data-target="#updateModal">
                                                <i class="fas fa-edit text-secondary"></i>
                                            </a>
                                            <span>
                                            <a href="#" wire:click="deleteConfirm({{ $dt->id_pelayanan }})"  data-bs-toggle="tooltip"
